...
stt-cursor/
  ...
  .agents/skills/          # Скилы проекта (см. раздел «Скилы проекта»)
  model/                   # Модель распознавания речи Vosk (в gitignore)
```

## Скилы проекта

В `.agents/skills/` лежат скилы, которые можно и нужно использовать:

- **idea-grid** — когда нужно пространство вариантов: изобретательские задачи, «предложи идеи/варианты», выбор конфигурации со многими параметрами. Не для отладки и задач с одним очевидным решением.
- **solution-search** — когда задача допускает несколько подходов и цена ошибки заметна (архитектура, миграция, оптимизация), два подхода подряд не сработали, или нужно лучшее/нестандартное решение.
- **git-commit-writer** — когда нужно составить commit message по diff: строго по фактам изменений, без выдумки, формат `<type>(<scope>): <description>` на английском. Используй при подготовке коммитов в этом проекте.
...
## Принципы ответов ассистента

- Отвечай на уровне senior-специалиста соответствующего домена, даже если
  вопрос сформулирован упрощённо: молча восстанови профессиональную
  формулировку и отвечай на неё.
- В сложных задачах первым предложением назначь себе конкретную экспертную
  роль, релевантную домену задачи.
- Никогда не теряй числа, версии, лимиты и запреты из условия задачи.
- Если решение опирается на быстро меняющийся инструмент/API — перед
  финальным ответом сверься с актуальной документацией.
